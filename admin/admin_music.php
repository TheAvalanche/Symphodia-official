...
<html>
  <head>
	...
	function processForm() {
		 var errors = '';
		 // album_id
		 var name = $("#add-form [name='album_id']").val();
		 if (!name) {
		  errors += ' - А ну ввели id альбома!!!\n';
		 }
	     // Validate header
	...
	function fillForm(str) {
		...
		$.ajax({
			type: "POST",
			url: "load.php",
			data: {id: str, table: "music"},
			success: function(data) {

				var pieces = data.split('//-//');
								
				var id = pieces[0];
				var album_id = pieces[1];
				var title = pieces[2];
				var content = pieces[3];
				var file = pieces[4];

				document.getElementById('update-id').value=id;
				document.getElementById('update-album_id').value=album_id;
				document.getElementById('update-title').value=title;
				document.getElementById('update-file').value=file;
  ...
  <body>
	...
	<div class="container">
		...
		<div class="hero-unit">
			<input id="add-music" type="hidden" name="table" value="music">
			
			<input id="add-album_id" type='text' name="album_id" placeholder="Номер альбома гоните." style="width: 500px;">
			...
			<textarea id="add-file" name="file" placeholder="Сюда вставлять ссылку на музыку..." style="width: 810px; height: 100px"></textarea>
		...
		<?php
			...
			$sql="SELECT id, title FROM music ORDER BY id DESC";
			...
			if (!$result)
			  ...
			  echo "<option value=\"\">Выбери композицию</option>";
		...
		<div class="hero-unit">
			<input id="update-music" type="hidden" name="table" value="music">
			<input id="update-delete" type="hidden" name="delete">
			<input id="update-id" type="hidden" name="id">
			
			<input id="update-album_id" type='text' name="album_id" placeholder="Номер альбома гоните." style="width: 500px;">
			...
			<textarea id="update-file" name="file" placeholder="Сюда вставлять ссылку на музыку..." style="width: 810px; height: 100px"></textarea>
